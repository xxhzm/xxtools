  ...
  <div class="container">
    ...
    <div class="tool-header">
      ...
      <div class="header-text">
        <h1>TCP端口检测</h1>
        <p class="subtitle">检测服务器端口连通性和响应时间</p>
      </div>
    </div>
      ...
      <div v-if="result" class="result-section">
        <div class="result-card">
          ...
          <div class="result-item">
            ...
            <span class="value">{{ result.port }}</span>
          </div>
          <div class="result-item">
            <span class="label">响应时间：</span>
            <span class="value" :class="getTimeType(parseInt(result.ping))">
              {{ result.ping }}
            </span>
          </div>
        </div>
...
// 从data.json导入工具信息
const toolInfo = toolsData.tools.find((tool) => tool.id === 3);

// 更新页面标题和描述
const title = toolInfo?.title || "TCP端口检测";
const description = toolInfo?.description || "检测服务器端口连通性和响应时间";
...
const validatePort = (value: string) => {
  // 只允许输入数字
  port.value = value.replace(/[^\d]/g, "");
  // 限制范围在1-65535
  const num = parseInt(port.value);
  if (num > 65535) {
    port.value = "65535";
  } else if (num < 1 && port.value !== "") {
    port.value = "1";
  }
};
...
const startCheck = async () => {
  ...
  try {
    const response = await fetch(
      `https://v2.xxapi.cn/api/tcping?address=${encodeURIComponent(
        address.value
      )}&port=${port.value}`
    );
